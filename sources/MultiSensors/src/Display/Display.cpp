...
namespace Display
{
    struct DMeasure
    {
        ...
        Measure::E name;
        Measure value;              // Ïîñëåäíåå óñòàíîâëåííîå çíà÷åíèå
        int position;               // Òåêóùàÿ îòğèñîâûâàåìàÿ ïîçèöèÿ
        uint time;                  // Âğåìÿ ïîñëåäíåãî èçìåíåíèÿ òåêóùåé îòğèñîâûâàåìîé ïîçèöèè
    ...
    namespace Buffer
    {
        static uint8 buffer[WIDTH * HEIGHT / 2];       // ×åòûğ¸õáèòíûé öâåò
...
namespace Display
{
    ...
    static bool mode_compass = false;        // Åñëè true - ğèñóåì êîìïàñ
...
void Display::DrawMeasures(uint)
{
    ...
    {
        String<>("ÓÃÎË ÏÎËÍÛÉ").Draw(x0, 70, Color::WHITE);
        String<>("ÓÃÎË ÎÒÍ").Draw(x0, 90);
        String<>("ÓÃË ÑÊÎĞÎÑÒÜ").Draw(x0, 110);
