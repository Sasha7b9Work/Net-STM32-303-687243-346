...
BaseMQ::BaseMQ(uint8 pin, uint8 pinHeater) {
  _pin = pin;
...
float BaseMQ::calculateResistance(int sensorADC) const {
  float sensorVoltage = (float)(sensorADC * (OPERATING_VOLTAGE / ADC_VALUE_MAX));
  float sensorResistance = (float)((OPERATING_VOLTAGE - sensorVoltage) / sensorVoltage * getRL());
  return sensorResistance;
}

float BaseMQ::readScaled(float a, float b) const {
  float ratio = readRatio();
  return std::expf((std::log(ratio) - b) / a);
}
